...
SECTION_LABELS = {
    ...
    'СПСиА': 'Пожарная сигнализация и автоматика',
    'АУГПТ': 'Автоматическое газовое пожаротушение',
    'КС': 'Кабельные сети',
    'ГОЧС': 'Мероприятия ГОЧС',
    'ОЗДС': 'Охранно-защитная дератизационная система',
    'ЭГ': 'Заземление и молниезащита',
    'ВНС': 'Насосная станция',
...
# Шифр раздела бывает со строчными буквами: СПСиА, АУГПТ. Прежний
# [А-ЯЁ]{2,5} их не брал, и раздел показывался как «не распознан».
CIPHER_PAT = re.compile(r'(ПР[-\s_][\w\.\-_/]*?-([А-ЯЁ][А-Яа-яЁё]{1,6}\d*))(?=[\s_(\[\.]|$)')


# Раздел сравнивается в верхнем регистре, а подпись ищется без учёта регистра:
# «СПСиА» в шифре и «СПСИА» после нормализации — один и тот же раздел.
_LABELS_UPPER = {k.upper(): v for k, v in SECTION_LABELS.items()}
...
def parse_filename(filename):
    ...
    label = ''
    if section:
        label = _LABELS_UPPER.get(section, '')
        if not label:
            base = re.sub(r'\d+$', '', section)
            label = _LABELS_UPPER.get(base, section)
